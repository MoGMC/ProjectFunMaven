			...
			if (sender instanceof Player) {
				...
				if (!player.hasPermission(getPerm())) {
					...
					sender.sendMessage(ChatColor.GOLD
							+ "ProjectFun by Nomar and Ploffie.");
					if (sender.hasPermission("projectfun.fun")) {
						sender.sendMessage(ChatColor.AQUA + "/fun"
								+ ChatColor.GOLD + ": Base command.");
					}
					if (sender.hasPermission("projectfun.hug")) {
						sender.sendMessage(ChatColor.AQUA + "/hug <player>"
								+ ChatColor.GOLD + ": Hug a player.");
					}
					if (sender.hasPermission("projectfun.lick")) {
						sender.sendMessage(ChatColor.AQUA + "/lick <player>"
								+ ChatColor.GOLD + ": Lick a player.");
					}
					if (sender.hasPermission("projectfun.slap")) {
						sender.sendMessage(ChatColor.AQUA + "/slap <player>"
								+ ChatColor.GOLD + ": Slap a player.");
					}
					if (sender.hasPermission("projectfun.rekt")) {
						sender.sendMessage(ChatColor.AQUA
								+ "/rek <player>"
								+ ChatColor.GOLD
								+ ": Rek a player like the Call of Duty kids would.");
					}
					sender.sendMessage(ChatColor.GOLD
					...
					if (sender.hasPermission("projectfun.rainbow")) {
						sender.sendMessage(ChatColor.AQUA + "/rainbow"
								+ ChatColor.GOLD + ": Toggle rainbow mode.");
					}
					if (sender.hasPermission("projectfun.hashtags.use")) {
						sender.sendMessage(ChatColor.AQUA + "/hashtag"
								+ ChatColor.GOLD + ": Get a list of HashTags.");
					}
