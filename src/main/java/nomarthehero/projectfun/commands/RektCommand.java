			...
			if (sender instanceof Player) {
				...
				if (!player.hasPermission(getPerm())) {					
					...
					if(PF.hugCool.containsKey(sender.getName())) {
						secondsLeft = ((PF.rektCool.get(sender.getName())/1000+cooldownTime) - System.currentTimeMillis()/1000);
						if(secondsLeft>0) {
							sender.sendMessage(ChatColor.RED + "There's no way you rek people that quick.");
							return true;
						}
					}
						...
						if (target != null) {
							Bukkit.broadcastMessage(ChatColor.LIGHT_PURPLE + p + ChatColor.DARK_RED + " REKT " + ChatColor.LIGHT_PURPLE + target.getName() + ChatColor.DARK_RED + "!");
							} else if(args[0].equalsIgnoreCase("all")) {
								Bukkit.broadcastMessage(ChatColor.LIGHT_PURPLE + p + ChatColor.DARK_RED + " REKT " + ChatColor.LIGHT_PURPLE + "EVERYONE" + ChatColor.DARK_RED + "!");
